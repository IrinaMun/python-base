"""
4. Написать скрипт, который выводит статистику для заданной папки в виде словаря, в котором ключи — верхняя граница
размера файла (пусть будет кратна 10), а значения — общее количество файлов (в том числе и в подпапках),
размер которых не превышает этой границы, но больше предыдущей (начинаем с 0), например:
    {
      100: 15,
      1000: 3,
      10000: 7,
      100000: 2
    }
Тут 15 файлов размером не более 100 байт; 3 файла больше 100 и не больше 1000 байт...
Подсказка: размер файла можно получить из атрибута .st_size объекта os.stat.
"""

import os
from os import path


def recursive_search_files(search_path, result_dict):
    for data in os.listdir(search_path):
        data = path.join(search_path, data)
        if path.isdir(data):
            recursive_search_files(data, result_dict)
        elif path.isfile(data):
            size = os.stat(data).st_size
            order = 10**len(str(size))
            if not result_dict.get(order):
                result_dict[order] = 0
            result_dict[order] += 1


all_files = {}

recursive_search_files(os.getcwd(), all_files)

print({k: all_files[k] for k in sorted(all_files)})
